...
.best-method { background: linear-gradient(145deg, #10b981 0%, #059669 100%) !important; box-shadow: 0 15px 40px rgba(16,185,129,0.4) !important; }
</style>
...
with st.sidebar:
    ...
    edited_df = st.data_editor(st.session_state.locations_df, num_rows="dynamic", column_config={"ID": st.column_config.NumberColumn("ID", disabled=True), "Demand_kg": st.column_config.NumberColumn("Demand (kg)")}, use_container_width=True)
...
if st.session_state.get('run_optimization', False):
    ...
    cols = st.columns(6)
    with cols[0]:
        st.markdown(f'<div class="metric-card best-method"><h4>{best_method}</h4><h2>🥇 #1</h2></div>', unsafe_allow_html=True)
    ...
    with cols[5]: st.metric("🌧️ Risk", "🟢" if st.session_state.best_result['flood_risk'] < 0.3 else "🟡" if st.session_state.best_result['flood_risk'] < 0.7 else "🔴")
    ...
    for rank, (method, data) in enumerate(sorted(method_analysis.items(), key=lambda x: x[1]['cost']), 1):
        comp_data.append({f"#{rank}": method, "Biaya": f"Rp {int(data['cost']):,}", "Jarak": f"{data['dist']:.1f} km", "Truk": data['truk'], "Flood": f"{data['flood_risk']:.2f}"})
    ...
    with col2:
        ...
        def create_excel():
            ...
            with pd.ExcelWriter(buffer, engine='openpyxl') as writer:
                pd.DataFrame(comp_data).to_excel(writer, sheet_name='6_Algoritma', index=False)
                pd.DataFrame(st.session_state.best_result['details']).to_excel(writer, sheet_name='Rute_Terbaik', index=False)
                pd.DataFrame(weather_rows).to_excel(writer, sheet_name='Cuaca', index=False)
